...
trait DbService {
   ...
   */
  def listSpecies: Try[List[Species]]
   ...
   * @return Count of rows updated
   */
  def markAllUsed(speciesList: List[Species]): Try[Int]
   ...
   * */
  def updateGbifData(species: Species): Try[Int]
   ...
   * */
  def setLineage(species: Species): Try[Species]
