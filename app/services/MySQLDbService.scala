...
object MySQLDbService extends DbService {
  ...
  override def markAllUnused: Int = {
    val query:String = "UPDATE system SET used_in_main=false;"
    val statement: Statement = connection.createStatement
    statement.executeUpdate(query)
  }

  override def markAllUsed(speciesList:List[Species]): Int = {
  ...
  override def deleteAllGBIFData(): Int = {
    val query:String =
      """
  ...
  override def updateGbifData(species:Species): Int = {
    val query:String =
      """
  ...
  override def setLineage(species:Species): Species = {
